...
class TestEynnydWebappGlobalHandlers(unittest.TestCase):
    class StubRequest(AbstractRequest):
    ...
    def test_global_method_handler(self):
        spy_handler = TestEynnydWebappHandlers.SpyHandler()
        routes = RoutesBuilder().add_global_method_handler("GET", spy_handler.test_handler).build()
        test_app = EynnydWebappBuilder().set_routes(routes).build()
        request = TestEynnydWebappHandlers.StubRequest(method="GET", request_uri="/foo/bar")
        response = test_app.process_request_to_response(request)
    ...
    def test_global_method_handler_ignored_for_specific_selection(self):
        spy_global_handler = TestEynnydWebappHandlers.SpyHandler("NOPE")
        spy_specific_handler = TestEynnydWebappHandlers.SpyHandler("YEP")
        routes = \
            RoutesBuilder() \
                .add_global_method_handler("GET", spy_global_handler.test_handler) \
                .add_handler("GET", "/fizz/buzz", spy_specific_handler.test_handler) \
                .build()
        test_app = EynnydWebappBuilder().set_routes(routes).build()
        request = TestEynnydWebappHandlers.StubRequest(method="GET", request_uri="/fizz/buzz")
        response = test_app.process_request_to_response(request)

        self.assertEqual(1, spy_specific_handler.handler_call_count)
        self.assertEqual(0, spy_global_handler.handler_call_count)
        self.assertEqual(b"YEP", response.body.content)

    def test_global_method_handler_only_for_method(self):
        spy_global_handler = TestEynnydWebappHandlers.SpyHandler("NOPE")
        spy_post_handler = TestEynnydWebappHandlers.SpyHandler("YEP")
        routes = \
            RoutesBuilder() \
    ...
    def test_global_method_handler_404s_by_method(self):
        spy_global_handler = TestEynnydWebappHandlers.SpyHandler("NOPE")
        routes = \
            RoutesBuilder() \
                .add_global_method_handler("GET", spy_global_handler.test_handler) \
